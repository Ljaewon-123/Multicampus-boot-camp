...
def click_bs(request):
    ...
    area_win = a[14]['면적 승수']
    popu_win = a[15]['세대 승수']
    json_dict = {'my_bs': key,'bs_cnt':b[key][0], 'bs_times':b[key][1], 'bs_km':b[key][2], 'bs_gagu':b[key][3], 'area_win': area_win, 'popu_win': popu_win}
    return JsonResponse(json_dict)
    # return http...(f'{key}의 수는 {b[key][0]}개이며 서울시 평균의 {b[key][1]}배이다,10km면적당 {key}개수는 {b[key][2]}개이고 만가구당 우리 구의 {key} 개수는 {b[key][3]}개 이다')
