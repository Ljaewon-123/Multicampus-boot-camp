...
def drawbargraph(file_name, user_loc):
    ...
    if file_name == 'pet_city_park' or file_name == 'pet_hospital' or file_name == 'pet_medical':
        ...
        buf = io.BytesIO()
        a_png = a.get_figure()
        a_png.savefig(buf, format='png',facecolor='#F6F5EF')
        buf.seek(0)
        ...
        plt.cla()
        plt.figure()
        # fig.patch.set_facecolor('xkcd:#f3eff4')
        lst = list(df.columns)
        bs_index = lst.index(user_loc)
        ...
        buf = io.BytesIO()
        b_png = b.get_figure()
        b_png.savefig(buf, format='png',facecolor='#F6F5EF')
        buf.seek(0)
...
def drawgraph():
    ...
    fig.savefig(buf, format='png',facecolor='#F6F5EF')
    buf.seek(0)
...
def drawpie():
    ...
    name_sort_be = df_industry_sort.columns.tolist()

    color_lst = ['#FF6A5A', '#FFBE59', '#CCCCFF', '#5EFF98', '#92EAFF', '#5B52FF', '#E12EFF', '#FF81B5', '#FF004E',
                 '#85DF9D']
    industry_sort = list()
    name_sort = list()
    industry_sort_be = df_industry_sort.T[0].tolist()
    for i in range(len(name_sort_be)):
        if (name_sort_be[i] != '약국') & (name_sort_be[i] != '공원'):
            industry_sort.append(industry_sort_be[i])
            name_sort.append(name_sort_be[i])
    print(len(name_sort))
    ...
    bs_sort_be = df_bs_sort.columns.tolist()
    survey_sort_be = df_bs_sort.T[0].tolist()

    survey_sort = list()
    bs_sort = list()
    for i in range(len(bs_sort_be)):
        if (bs_sort_be[i] != '약국') & (bs_sort_be[i] != '공원'):
            survey_sort.append(survey_sort_be[i])
            bs_sort.append(bs_sort_be[i])
    ...
    fig2 = plt.gcf()
    buf = io.BytesIO()
    fig2.savefig(buf, format='png', facecolor='#F3EFF4')
    buf.seek(0)
    string = base64.b64encode(buf.read())
    return string
...
def drawbox():
    ...
    fig2 = plt.gcf()
    buf = io.BytesIO()
    fig2.savefig(buf, format='png', facecolor='#EFF4F3')
    buf.seek(0)
    string = base64.b64encode(buf.read())
    return string
