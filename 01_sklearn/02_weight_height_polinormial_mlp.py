...
# 3. 준비
# linear = MLPRegressor(hidden_layer_sizes=[10,2],max_iter=500)  # 몇이 디폴트??? 각 매배변수 무엇??[100]
linear = MLPRegressor(activation='relu',hidden_layer_sizes=(200,2),solver='adam') # solver 경사 하강법 알고리즘
# 하이퍼 파라미터 튜닝
# 머신러닝에서 파라미터는 가중치(W),편향(b)같은 학습과정에서 모델이 자동으로 업글하면 갱신하는 값
# 하이퍼 파라미터는 그외 연구자가 수정할수 있는값 학습률,optimizer,활성화함수,손실함수등 다양한 값들을 가르킴
